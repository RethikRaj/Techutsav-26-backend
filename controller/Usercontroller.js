...
export const loginUser = async (req, res) => {
  try {
    ...
    if (!email || !password) {
      return res
        .status(400)
        .json(standardResponse(400, "Email and password required"));
    }
    ...
    if (!user) {
      return res
        .status(404)
        .json(standardResponse(404, "User not found"));
    }

    if (!user.isEmailVerified) {
      return res
        .status(403)
        .json(standardResponse(403, "Email not verified"));
    }
    ...
    if (!match) {
      return res
        .status(401)
        .json(standardResponse(401, "Incorrect password"));
    }
    ...
    const token = generateLoginToken(payload, process.env.loginSecret);
    ...
    return res.status(200).json(
      standardResponse(200, "Login successful", payload)
    );
  } catch (err) {
    console.log(err);
    return res
      .status(500)
      .json(standardResponse(500, "Login failed"));
  }
};
